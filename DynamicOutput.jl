...
# ╔═╡ 4fd8a3f9-8b4a-43a1-9767-502d27ad5c5e
begin
	using PlutoUI, DataFrames, Statistics, Plots, CSV
	# Load/prepare your data here:
	const working_directory = "C:/Users/dmayerh/Onedrive - Personal/OneDrive/DATIpilot/Inhaltliches/Papers/Hirschman/SimOutput"

	workingdf = CSV.read(joinpath(working_directory, "Rho4.csv"), DataFrame)
	# Or if df is already in memory, just use it.

	# Slider grids (pick one)
	avalues = sort(unique(workingdf.alpha))      # or: collect(alphalist)
	bvalues = sort(unique(workingdf.beta))       # or: collect(betalist)

	const EPS = 1e-9
end

# ╔═╡ cd249a1f-2ed7-4ac0-9c83-9cba8d976ec9
using PlutoSliderServer

# ╔═╡ 80a7223e-773e-11f0-22b4-d770b861fd74
begin
	has_rows(a, b) = nrow(filter(r ->
	    isapprox(r.alpha, a; atol=EPS) &&
	    isapprox(r.beta,  b; atol=EPS), workingdf)) > 0

	function find_initial_indices()
	    for (ai, a) in enumerate(avalues), (bi, b) in enumerate(bvalues)
	        if a + b <= 1 && has_rows(a, b)
	            return ai, bi
	        end
	    end
	    return 1, 1  # fallback if nothing matches
	end
	ialpha0, ibeta0 = find_initial_indices()
end


# ╔═╡ b44d9018-6c4b-4dd4-8b9d-0147b81981e6
@bind i_alpha Slider(1:length(avalues); default=ialpha0, show_value=true)

# ╔═╡ 6fd4a5c5-9945-4675-8395-a0f4b751c3b4
@bind i_beta  Slider(1:length(bvalues); default=ibeta0,  show_value=true)

# ╔═╡ e0e55318-864c-4228-bd5d-24372b134db9
begin
	alpha_selected = avalues[i_alpha]
	beta_selected  = bvalues[i_beta]
end

# ╔═╡ 98463d29-969b-47fd-8b4a-fd567bfe3499
begin
	a = alpha_selected; b = beta_selected
	if a + b > 1
	    "Skipping (α=$(a), β=$(b)) because α+β>1"
	else
	    sel = filter(r -> isapprox(r.alpha, a; atol=EPS) &&
	                      isapprox(r.beta,  b; atol=EPS), workingdf)
	    if nrow(sel) == 0
	        "No data for α=$(a), β=$(b))"
	    else
	        g = combine(groupby(sel, :sigma), :utilitysum => mean => :mean_utilitysum)
	        plt = scatter(sel.sigma, sel.utilitysum;
	            xlabel="sigma", ylabel="utilitysum",
	            title="utilitysum vs sigma (α=$(a), β=$(b))",
	            legend=false, markerstrokewidth=0.5, markersize=6)
	        plot!(g.sigma, g.mean_utilitysum; lw=2, marker=:circle, markersize=7, label="mean utilitysum")
	        plt
	    end
	end
end
...
# ╔═╡ 8c142a0f-2240-4230-9418-e21f61b614a1
PlutoSliderServer.export_notebook(
    "DynamicOutput.jl";                         # your Pluto notebook file
    Export_output_dir = working_directory
)

# ╔═╡ edcbcf66-6c91-4916-b834-3df1d6e162f5
working_directory
...
manifest_format = "2.0"
project_hash = "09ea139d895fc006543f815a5a756d3457e9bf5e"
...
git-tree-sha1 = "0885a35db1e99f9ad039cd3a1a8fe09cc43f38b4"
